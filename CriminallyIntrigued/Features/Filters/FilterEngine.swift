...
/// Minimal surface the filter engine needs. `CaseEntry` conforms for free;
...
protocol CaseFilterable {
    var title: String { get }
    var summary: String? { get }
    var victimCount: Int? { get }
    var countryName: String? { get }
    var startDate: Date? { get }
}

extension CaseEntry: CaseFilterable {}
...
enum FilterEngine {
    /// Applies search → filters → sort. Entries with unknown victim count are
    /// *excluded* from a victim-count filter, never coerced to 0 (spec §9.3).
    static func apply<T: CaseFilterable>(_ items: [T], _ criteria: FilterCriteria) -> [T] {
        var result = items
        ...
        if let threshold = criteria.minVictimCount {
            result = result.filter { ($0.victimCount ?? Int.min) >= threshold }
        }
    ...
    static func sort<T: CaseFilterable>(_ items: [T], by option: SortOption) -> [T] {
        switch option {
        case .alphaAscending:
    ...
    static func availableCountries<T: CaseFilterable>(_ items: [T]) -> [String] {
