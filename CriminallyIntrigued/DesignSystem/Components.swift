...
/// A single metadata pill (country, victim count, year). Rendered only when the
...
struct CaseRow: View {
    ...
    @ViewBuilder private var thumbnail: some View {
        if AppConfig.showArticleImages, let url = entry.thumbnailURL {
    ...
    private var chips: [Chip] {
        var result: [Chip] = []
        if let country = entry.countryName {
            result.append(Chip(symbol: "globe", text: country))
        }
        if let victims = entry.victimCount {
            result.append(Chip(symbol: "person.2", text: "\(victims)"))
        }
        if let year = entry.startDate.map(Self.yearFormatter.string(from:)) {
