...
final class AppEnvironment {
    ...
    init() {
        ...
        do {
            container = try ModelContainer(for: CaseEntry.self)
        } catch {
            ...
            container = try! ModelContainer(for: CaseEntry.self, configurations: config)
        }
        self.modelContainer = container

        let context = container.mainContext
        let http = HTTPClient()
        let wikidata = WikidataService(http: http)
        let wikipedia = WikipediaService(http: http)

        self.catalog = CatalogRepository(context: context, wikidata: wikidata, wikipedia: wikipedia)
